...
class ChangeTrackingRunner 
	CHANGE_TRACKING_FILE = "/var/opt/Microsoft/omsagent/tmp/ChangeTrackingInventory.xml"
	CHANGE_TRACKING_STATE_FILE = "/var/opt/microsoft/omsagent/state/ChangeTrackingHash"
	PREV_HASH = "PREV_HASH"
	...
	def self.transform_and_wrap()
		if File.exist?(CHANGE_TRACKING_FILE)
			...
			@@log.debug "ChangeTracking : Packages x #{output["packages"].size},
                                          Services x #{output["services"].size}, 
                                          Files x #{output["fileInventories"].size}"
